            ...
            >
                <article className="flex h-full items-center px-8 gap-4">
                    ...
                    <div className="flex flex-col flex-grow w-2/3">
                        <Link to={`/blog/${slug}`} className="text-lg font-semibold leading-snug text-gray-900 hover:text-rose-600 mb-2 line-clamp-2">
                            {title}
                        </Link>
                        {/* Publish date badge */}
                        <div className="mt-auto flex flex-col items-start">
                            <p className="text-xs font-medium mb-2 text-gray-700">{formatDate(publish)}</p>
                            <hr className='bg-black w-[4%] h-[3px]'/>
                            <div className='text-sm leading-snug text-gray-900 mt-2 line-clamp-1' dangerouslySetInnerHTML={{ __html: content }} />
                        </div>
                    </div>
                </article>
            </div>
